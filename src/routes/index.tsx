...
/* ---------- 10. DOCUMENT COMPRESSION WORKFLOW ---------- */
function DocumentWorkflow() {
  const steps = [
    {
      icon: "Upload",
      title: "Document Ingestion",
      description: "Batch upload from scanners, email, or existing repositories. Supports PDF, TIFF, JPEG, and legacy formats.",
      color: "#0049D7",
    },
    {
      icon: "Scan",
      title: "AI-Powered OCR",
      description: "Multi-language OCR with 99.2% accuracy. Handles Bahasa Malaysia, English, and aged/damaged documents.",
      color: "#0049D7",
    },
    {
      icon: "Cpu",
      title: "Intelligent Compression",
      description: "Proprietary ML model compresses documents up to 70% without quality loss. Optimizes for storage and speed.",
      color: "#FFB400",
    },
    {
      icon: "Search",
      title: "Smart Indexing",
      description: "Auto-tagging, metadata extraction, and full-text search. Find any document in seconds.",
      color: "#10B981",
    },
    {
      icon: "Lock",
      title: "Secure Access",
      description: "Role-based permissions, audit trails, and PDPA compliance built-in at every step.",
      color: "#0049D7",
    },
    {
      icon: "Zap",
      title: "Instant Retrieval",
      description: "Compressed documents load 40% faster. View, edit, and share without decompression delays.",
      color: "#FFB400",
    },
  ];

  return (
    <section className="relative overflow-hidden bg-[#F4F7FB] px-6 py-24">
      <div className="mx-auto max-w-7xl">
        <ScrollReveal>
          <div className="mx-auto max-w-3xl text-center">
            <SectionTag>How It Works</SectionTag>
            <h2 className="mt-5 font-display text-4xl font-extrabold sm:text-5xl">
              From Paper to <span className="text-gradient-cg">Digital Intelligence</span>
            </h2>
            <p className="mt-5 text-lg text-[#5B6478]">
              Our AI-powered workflow transforms millions of documents into searchable, compressed, and compliant digital assets in minutes.
            </p>
          </div>
        </ScrollReveal>
            ...
            {steps.map((step, i) => {
              ...
              return (
                <ScrollReveal key={step.title} delay={i * 0.05}>
                  <div className="relative overflow-hidden rounded-2xl border border-[rgba(0,73,215,0.1)] bg-white p-6 shadow-sm transition hover:shadow-md">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-heading text-base font-semibold text-[#0B1B3D]">{step.title}</h3>
                        <p className="mt-2 text-sm leading-relaxed text-[#5B6478]">{step.description}</p>
                      </div>
                    </div>
                    <div className="absolute right-4 top-4 text-[10px] font-bold text-[rgba(0,73,215,0.2)]">
        ...
        <ScrollReveal delay={0.3}>
          <div className="mt-20 rounded-3xl border border-[rgba(0,73,215,0.15)] bg-gradient-to-br from-[#0B1B3D] to-[#1a2e4d] p-8 shadow-2xl">
            <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-[1.2fr,0.8fr]">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full border border-[#0049D7]/30 bg-[#0049D7]/10 px-4 py-1.5">
                  <Icons.Building2 className="h-4 w-4 text-[#0049D7]" />
                  <span className="text-xs font-semibold uppercase tracking-wider text-[#0049D7]">Enterprise Case Study</span>
                </div>
                <h3 className="mt-5 font-display text-3xl font-extrabold text-white">
                  VFS Global: Processing <span className="text-[#FFB400]">Millions</span> of Visa Documents
                </h3>
                <p className="mt-4 text-lg leading-relaxed text-white/80">
                  VFS Global, the world's largest visa outsourcing company, processes millions of visa applications daily. Our SmartForce DMS handles their document compression and digitalization workflow across 140+ countries.
                </p>
                
                <div className="mt-8 grid grid-cols-2 gap-6">
                  {[
                    { value: "12M+", label: "Documents processed daily" },
                    { value: "140+", label: "Countries covered" },
                    { value: "65%", label: "Storage cost reduction" },
                    { value: "99.9%", label: "Uptime guaranteed" },
                  ].map((stat) => (
                    <div key={stat.label}>
                      <p className="font-display text-3xl font-extrabold text-[#FFB400]">{stat.value}</p>
              ...
              <div className="relative">
                <div className="pointer-events-none absolute -inset-4 rounded-2xl bg-[#0049D7]/20 blur-2xl" />
                <div className="relative rounded-2xl border border-[rgba(255,255,255,0.1)] bg-[rgba(255,255,255,0.05)] p-6 backdrop-blur">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="h-10 w-10 rounded-full bg-gradient-to-br from-[#0049D7] to-[#FFB400]" />
                    <div>
                      <p className="font-heading text-sm font-semibold text-white">VFS Global Integration</p>
                      <p className="text-xs text-white/60">Live since 2023</p>
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                    {[
                      "Batch ingestion from visa centers worldwide",
                      "Multi-language OCR for 50+ languages",
                      "Compressed archives with instant retrieval",
                      "GDPR & PDPA compliant data handling",
                      "Real-time sync with backend systems",
                    ].map((item, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <Icons.CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />
